...
// 优先使用本机已安装的 Chrome，避免下载 Playwright 专用 Chromium
const SYSTEM_CHROME =
...
const ensurePlaywrightBrowser = async () => {
  try {
    const browser = await chromium.launch({ executablePath: SYSTEM_CHROME });
    await browser.close();
  } catch (error) {
    throw new Error(
      `Failed to launch browser at ${SYSTEM_CHROME}. Set PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH env var to your Chrome/Edge path.`,
      { cause: error }
    );
  }
};
