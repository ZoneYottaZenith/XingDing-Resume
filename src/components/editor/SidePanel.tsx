...
function SettingCard({
  ...
  return (
    ...
    >
      <CardHeader className="p-3 pb-0 md:p-4 md:pb-0 flex flex-row items-center justify-between space-y-0">
...
export function SidePanel() {
  ...
  return (
    <motion.div
      className={cn(
        "w-full border-r md:overflow-y-auto",
    ...
    >
      <div className="p-3 md:p-4 space-y-3 md:space-y-4">
        ...
        >
          <div className="flex flex-wrap gap-2 md:gap-2.5 pt-1">
            {THEME_COLORS.map((presetTheme) => (
              <button
                key={presetTheme}
                className={cn(
                  "relative group w-5 h-5 md:w-6 md:h-6 rounded-full overflow-hidden transition-all duration-200 focus:outline-none",
        ...
        <SettingCard icon={Zap} title={t("mode.title")}>
          <div className="flex gap-2 md:space-y-4 md:block">
            <div className="flex-1 flex flex-col items-center gap-1.5 md:flex-row md:items-center md:justify-between md:gap-4">
              <Label className="text-muted-foreground text-xs md:text-sm text-center md:text-left">
