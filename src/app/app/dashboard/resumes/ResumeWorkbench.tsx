...
export const ResumeWorkbench = () => {
    ...
    const importResumeFromJson = async (file: File) => {
        ...
        for (const item of list) {
            ...
            lastId = addResume(newResume);
        }

        if (lastId) setActiveResume(lastId);
        setIsImportDialogOpen(false);

        if (list.length === 1) {
            toast.success(t("dashboard.resumes.importSuccess"));
            if (lastId) router.push(`/app/workbench/${lastId}`);
        } else {
    ...
    return (
        <ScrollArea className="h-[calc(100vh-2rem)] w-full">
            <motion.div
                ...
                className="flex-1 space-y-3 py-4 md:space-y-6 md:py-8"
            >
                <motion.div
                ...
                >
                    {hasConfiguredFolder ? (
                        ...
                        <Alert
                            variant="destructive"
                            className="mb-3 md:mb-6 text-xs bg-red-50/50 dark:bg-red-950/30 border-red-200 dark:border-red-900"
                        >
                            <AlertCircle className="h-4 w-4" />
                            <AlertTitle>{t("dashboard.resumes.notice.title")}</AlertTitle>
                            <AlertDescription className="flex items-center justify-between">
                                <span className="text-red-700 dark:text-red-400">
                                    <span className="hidden md:inline">{t("dashboard.resumes.notice.description")}</span>
                                    <span className="md:hidden">{t("dashboard.resumes.notice.descriptionMobile")}</span>
                                </span>
                                <Button
                                    variant="outline"
                                    size="sm"
                ...
                <motion.div
                    className="px-3 sm:px-6 flex items-center justify-between"
                    initial={{ y: -20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                    transition={{ duration: 0.3 }}
                >
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        >
                            <Button
                                ...
                                variant="default"
                                className="h-8 md:h-10 px-3 md:px-4 text-xs md:text-sm bg-gray-900 text-white hover:bg-gray-800 dark:bg-primary dark:text-primary-foreground dark:hover:bg-primary/90"
                            >
                ...
                <motion.div
                    className="flex-1 w-full p-2 sm:p-6"
                    initial={{ y: 20, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                        ...
                        >
                            ...
                            >
                                <CardContent className="flex-1 p-0 text-center flex flex-col items-center justify-center">
                                    <motion.div
                                        className="mb-2 p-2 md:mb-4 md:p-4 rounded-full bg-gray-100 dark:bg-primary/10"
                                        whileHover={{ rotate: 90 }}
                                        transition={{ duration: 0.2 }}
                                    >
                                        <Plus className="h-5 w-5 md:h-8 md:w-8 text-gray-600 dark:text-primary" />
                                    </motion.div>
                                    <CardTitle className="text-sm md:text-xl text-gray-900 dark:text-gray-100 px-2 md:px-4">
                                        {t("dashboard.resumes.newResume")}
                                    </CardTitle>
                                    <CardDescription className="mt-1 text-[10px] md:text-sm text-gray-600 dark:text-gray-400 px-2 md:px-4">
