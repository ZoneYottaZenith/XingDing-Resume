  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 pt-12 pb-3 px-4 flex items-end border-t border-transparent z-10 transition-colors group-hover:bg-white/50 dark:group-hover:bg-gray-950/50">
            <div className="flex flex-col w-full">
              <span className="text-[11px] md:text-[15px] font-semibold truncate text-gray-900 dark:text-gray-100 drop-shadow-sm">
                {templateName}
              </span>
              <span className="text-[9px] md:text-[11px] text-gray-600 dark:text-gray-300 mt-0.5 font-medium truncate">
                {templateDescription}
              </span>
            </div>
          </div>
        ...
        <CardFooter className="pt-1.5 pb-1.5 px-1.5 md:pt-2 md:pb-2 md:px-2 bg-white dark:bg-gray-950 border-t border-gray-100 dark:border-gray-800 z-10">
          <div className="grid grid-cols-2 gap-1.5 md:gap-2 w-full">
            ...
            >
              <Button
                variant="outline"
                className="w-full text-xs md:text-sm hover:bg-gray-100 dark:border-primary/50 dark:hover:bg-primary/10"
                size="sm"
                onClick={(e) => {
                  e.stopPropagation();
                  onPreview();
            ...
            >
              ...
              >
                ...
                <span className="hidden sm:inline">{useTemplateLabel}</span>
              </Button>
            </motion.div>
          </div>
        </CardFooter>
...
const TemplatesPage = () => {
  ...
  const [dialogPreviewScale, setDialogPreviewScale] = useState(PREVIEW_MODAL_SCALE);

  useEffect(() => {
    if (!previewTemplate) return;
    // Dialog 打开后 DOM 需要一帧才稳定，用 RAF + setTimeout 确保测量准确
    const measure = () => {
      if (!dialogPreviewRef.current) return;
      const { width } = dialogPreviewRef.current.getBoundingClientRect();
      if (width > 10) setDialogPreviewScale(width / A4_WIDTH_PX);
    };
    const timer = setTimeout(() => requestAnimationFrame(measure), 50);
    const observer = new ResizeObserver((entries) => {
      const { width } = entries[0].contentRect;
      if (width > 10) setDialogPreviewScale(width / A4_WIDTH_PX);
    });
    // observe 在 setTimeout 后执行，确保 ref 已挂载
    const attachTimer = setTimeout(() => {
      if (dialogPreviewRef.current) observer.observe(dialogPreviewRef.current);
    }, 60);
    return () => {
      clearTimeout(timer);
  ...
  return (
    <ScrollArea className="h-[calc(100vh-2rem)] w-full">
      <div className="w-full max-w-[1600px] mx-auto py-4 md:py-8 px-3 sm:px-6">
        <div className="flex flex-col space-y-4 md:space-y-8">
            ...
            <div className="flex items-center space-x-2 bg-gray-50/50 dark:bg-gray-900/50 p-2 rounded-full border border-gray-100 dark:border-gray-800 backdrop-blur-sm self-start sm:self-auto overflow-x-auto">
              {PRESET_COLORS.map((color) => (
                <button
                  ...
                  className={cn(
                    "relative w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center shrink-0 transition-transform hover:scale-110",
          ...
          >
            {activePreviewTemplate && (
              <DialogContent className="w-[calc(100vw-32px)] max-w-[680px] p-0 overflow-hidden border-0 shadow-lg rounded-xl bg-white dark:bg-gray-900">
                <div className="flex flex-col">
                  ...
                  <div className="overflow-hidden flex items-center justify-center bg-gray-50 dark:bg-gray-950 py-4 md:py-8 pointer-events-none">
                    <div
                      ref={dialogPreviewRef}
                      className="relative bg-white shadow-md ring-1 ring-gray-200/50 overflow-hidden w-full mx-4 md:mx-0 md:w-[420px]"
                    ...
                    >
                      <div
                        ...
                        style={{
                          width: "210mm",
                          height: "297mm",
                          transform: `scale(${dialogPreviewScale})`,
                          transformOrigin: "top left",
